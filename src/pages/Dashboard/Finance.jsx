        ...
        <main className={`flex-1 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'} pt-20`}>
          <header className="fixed top-0 right-0 left-0 z-40 bg-white bg-opacity-95 backdrop-blur-md shadow border-b border-gray-200">
            <div className={`px-4 md:px-6 py-4 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'}`}>
              <div className="flex items-center gap-3">
                <button onClick={toggleSidebarOpen} className="md:hidden p-2 rounded-xl hover:bg-gray-100" aria-label="Abrir menú"><i className="fas fa-bars text-xl text-gray-700"></i></button>
                ...
                <div>
                  <h1 className="text-2xl font-Lato font-bold text-gray-800">Mensualidades</h1>
                  <p className="text-sm text-gray-600 font-Poppins">Pagadas, pendientes y vencidas</p>
                </div>
              </div>
            </div>
          </header>
          <section className="px-4 md:px-6 py-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="bg-white/90 backdrop-blur rounded-xl border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">Mensualidades: pagadas/pendientes/vencidas (placeholder)</div>
              <div className="bg-white/90 backdrop-blur rounded-xl border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">Proyección de ingresos (placeholder)</div>
              <div className="bg-white/90 backdrop-blur rounded-xl border border-gray-200 shadow-sm p-6 hover:shadow-md transition-shadow">Reportes por sede/fecha (placeholder)</div>
            </div>
          </section>
        </main>
      </div>
