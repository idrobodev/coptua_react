...
export const useFileManager = () => {
  ...
  const uploadFiles = async (acceptedFiles) => {
    ...
    try {
      for (const file of acceptedFiles) {
        await storageService.upload(file, state.currentPath);
      }
  ...
  const createFolder = async () => {
    ...
    try {
      await storageService.createFolder(folderOperations.newFolderName.trim(), state.currentPath);
  ...
  const renameFolder = async (oldName) => {
    ...
    try {
      await storageService.renameFolder(oldName, folderOperations.renamingFolder.newName.trim(), state.currentPath);
  ...
  const deleteFolder = async (folderName) => {
    ...
    try {
      await storageService.deleteFolder(state.currentPath ? `${state.currentPath}/${folderName}` : folderName);
  ...
  const processedFiles = state.files
    .filter(file => 
      file.name.toLowerCase().includes(ui.searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      let aValue, bValue;
